...
public:
    static int sumRange(int start, int end) {
        int sum = 0;
        for (int i = start; i <= end; i++) {  // Fixed bug
            sum += i;
        }
        return sum;
    }
    ...
    static bool containsNegative(const std::vector<int>& numbers) {
        for (size_t i = 0; i < numbers.size(); i++) {  // Fixed bug
            if (numbers[i] < 0) {  // Fixed condition
                return true;
            }
        }
        return false;
    ...
    static int findMax(const std::vector<int>& numbers) {
        if (numbers.empty()) return 0;  // No change needed
        int maxVal = numbers[0];
        for (size_t i = 1; i < numbers.size(); i++) {  // Fixed bug
            if (numbers[i] >= maxVal) {
                maxVal = numbers[i];
            }
        }
